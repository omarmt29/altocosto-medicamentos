"""
Scraper PA · Farmacias Julios → medicamentos_altos_costos_america

Cadena panameña con catálogo WooCommerce Store API (oncológicos y alto costo
con PVP público). Complementa Arrocha / Pan Am Farma.

Uso:
    python scrapper/pa_julios.py
    python scrapper/pa_julios.py --fresh
"""
...
CACHE = ROOT / "cache" / "farmacias" / "julios"
CATALOGO_CACHE = CACHE / "catalogo.json"
BASE = "https://www.farmaciasjulios.com"
...
def bajar_catalogo(forzar: bool = False) -> list[dict[str, Any]]:
    if CATALOGO_CACHE.exists() and CATALOGO_CACHE.stat().st_size > 20 and not forzar:
        ...
        if isinstance(data, list) and data:
            print(f"Catálogo Farmacias Julios en caché: {len(data)} productos")
            return data
    print(f"Descargando catálogo Farmacias Julios ({API}) …")
    session = requests.Session()
    session.headers.update({"User-Agent": UA, "Accept": "application/json"})
    out: list[dict[str, Any]] = []
    for page in range(1, MAX_PAGES + 1):
        r = session.get(API, params={"per_page": PER_PAGE, "page": page}, timeout=60)
        r.raise_for_status()
        data = r.json()
...
def filas_para_db(catalogo: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for p in catalogo:
        ...
        if not nombre or not precio:
            continue
        texto = f"{nombre} {desc} {tags} {cats} {url}"
        hits = medicamentos_que_pegan(texto)
        if not hits:
            continue
        fake = {
            "nombre": nombre,
            "principios_activos": [desc, tags, cats],
...
def main() -> int:
    ...
    print("=" * 64)
    print(" PA Farmacias Julios → medicamentos_altos_costos_america")
    print("=" * 64)
    ...
    print(
        ...
        f"PA Julios: {contar(PAIS, FARMACIA)} · total tabla: {contar()}"
    )
    by_med: dict[str, int] = {}
    for f in filas:
