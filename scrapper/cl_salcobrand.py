"""
Scraper CL · Salcobrand (Algolia) → P_aguila.medicamentos_altos_costos_america

Uso:
    python scrapper/cl_salcobrand.py
    python scrapper/cl_salcobrand.py --fresh
"""
...
CACHE = ROOT / "cache" / "farmacias" / "salcobrand"
BASE = "https://salcobrand.cl"
PAIS = "Chile"
FARMACIA = "Salcobrand"
MONEDA = "CLP"

ALGOLIA_APP = "GM3RP06HJG"
ALGOLIA_KEY = "0259fe250b3be4b1326eb85e47aa7d81"
...
def precio_hit(h: dict[str, Any]) -> float | None:
    """Precio de lista (normal_price); no usar direct_discount/promo."""
    for key in ("normal_price", "direct_discount"):
        raw = h.get(key)
        try:
            p = float(raw)
...
def url_pdp(h: dict[str, Any]) -> str:
    ...
    if slug:
        return f"{BASE}/products/{slug.lstrip('/')}"[:500]
    sku = str(h.get("sku") or h.get("id") or "").strip()
    if sku:
        return f"{BASE}/products/{sku}"[:500]
    return BASE
    ...
    for page in range(MAX_PAGINAS):
        ...
        for h in hits:
            if not isinstance(h, dict):
                continue
            pid = str(h.get("id") or h.get("objectID") or h.get("sku") or "")
...
def filas_de_hits(
    ...
    for h in hits:
        ...
        if not nombre:
            continue
        lab = str(h.get("brand") or "").strip()
        blob = " ".join(x for x in (nombre, lab, str(h.get("sku") or "")) if x)
        ...
        if not precio:
            continue
        pid = str(h.get("sku") or h.get("id") or h.get("objectID") or "")
        if not pid:
            continue
        p_fake = {
            "nombre": nombre,
        ...
        out.append(
            {
                ...
                "precio": precio,
                "moneda": MONEDA,
                "disponibilidad": "Disponible" if en_stock else "Agotado",
                "calidad": calidad,
                "observacion": obs,
...
def recolectar(*, forzar: bool = False, meds: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    ...
    if not red_ok and not filas:
        print("  Sin filas y red bloqueada.")
    return filas
...
def main() -> int:
    ...
    print("=" * 64)
    print(" CL Salcobrand → medicamentos_altos_costos_america")
    print("=" * 64)
    ...
    print(
        ...
        f"Chile Salcobrand: {contar(PAIS, FARMACIA)} · total tabla: {contar()}"
    )
    return 0
